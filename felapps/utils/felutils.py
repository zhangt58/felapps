...
#------------------------------------------------------------------------#
    ...
    def createMenu(self):
        ...
        ### Exit
        fileMenu = wx.Menu()
        self.saveItem = fileMenu.Append(wx.ID_ANY, u'&Save\tCtrl+S',
                                        u'Save data to file')
        self.loadItem = fileMenu.Append(wx.ID_ANY, u'&Load\tCtrl+L',
...
#------------------------------------------------------------------------#
    ...
    def createPanel(self):
        ...
        titlefont = wx.SystemSettings.GetFont(wx.SYS_SYSTEM_FONT)
        ...
        sbox1 = wx.StaticBox(
            self.panel, id=wx.ID_ANY, label=u'Harmonic Generation')
        ### config staticbox
        ...
        layoutBitmap = wx.StaticBitmap(
            self.panel, wx.ID_ANY,
            wx.BitmapFromImage(
                funutils.rescaleImage(resutils.moddslayouticon.GetImage(),
                                      1.0)))
        ...
        vbox.Add(
            sbsizer_layout,
            proportion=3,
            flag=wx.EXPAND | wx.ALIGN_CENTRE | wx.TOP | wx.LEFT | wx.RIGHT,
            border=15)
        ...
        sbox21 = wx.StaticBox(
            self.panel, id=wx.ID_ANY, label=u'Parameters Configuration')
        ### config staticbox
        ...
        self.paramslaserbtn = wx.Button(
            self.panel, id=wx.ID_ANY, label='Seedlaser')
        self.paramsebeambtn = wx.Button(
            self.panel, id=wx.ID_ANY, label='E-beam')
        self.paramsmodulatorbtn = wx.Button(
            self.panel, id=wx.ID_ANY, label='Modulator')
        self.paramschicanebtn = wx.Button(
            self.panel, id=wx.ID_ANY, label='Chicane')
        ...
        self.Bind(wx.EVT_BUTTON, self.OnConfigModulator,
                  self.paramsmodulatorbtn)
        self.Bind(wx.EVT_BUTTON, self.OnConfigChicane, self.paramschicanebtn)

        #### config buttons
        ##### button label font
        btnfont = wx.SystemSettings.GetFont(wx.SYS_SYSTEM_FONT)
        ...
        sbox22 = wx.StaticBox(
            self.panel, id=wx.ID_ANY, label=u'Longitudianl Phase-space')
        ...
        ## font for staticbox
        sb1font = wx.SystemSettings.GetFont(wx.SYS_SYSTEM_FONT)
        sb1font.SetPointSize(10)
        sb1font.SetWeight(wx.FONTWEIGHT_BOLD)
        sb1fontcolor = 'blue'

        #### modulated phasespace
        sbox221 = wx.StaticBox(
            self.panel,
            id=wx.ID_ANY,
            label=u'Modulated Beam',
        ...
        #### plotpanel for psmod
        self.psmodgraph = pltutils.ImagePanelxy(
            self.panel, figsize=(3.9, 3.9), dpi=60, bgcolor=None)
        sbsizer_psmod.Add(
            self.psmodgraph,
            proportion=1,
        ...
        sbsizer_phasespace.Add(
            sbsizer_psmod,
            proportion=1,
            flag=wx.EXPAND | wx.ALIGN_CENTER | wx.LEFT | wx.TOP | wx.BOTTOM,
            border=8)

        #### dispersed phasespace
        sbox222 = wx.StaticBox(
            self.panel,
            id=wx.ID_ANY,
            label=u'Dispersed Beam',
            style=wx.ALIGN_CENTER)
        ...
        #### plotpanel for pschi
        self.pschigraph = pltutils.ImagePanelxy(
            self.panel, figsize=(3.9, 3.9), dpi=60, bgcolor=None)
        sbsizer_pschi.Add(
            self.pschigraph,
            proportion=1,
...
# update phasespace after mod and chi
    ...
    def OnUpdatePS(self, psid='mod', newval=0):
        ...
        if psid == 'mod':  ## modulation
            ...
            modfilecmd = 'sed -i "s/aw0.*/aw0=' + str(
                au) + '/;s/awd.*/awd=' + str(au) + '/i" ../sim/mod.in'
            ...
            self.psmodgraph.x, self.psmodgraph.y = data[1, :], (
                data[0, :] - gam0) / sigg
            self.psmodgraph.repaint()

        elif psid == 'chi':  ## dispersed
            modfilecmd = 'sed -i "s/ibfield.*/ibfield=' + str(
                newval) + '/i" ../sim/modp.in'
            os.system(modfilecmd)
            os.system('cd ../sim; bash runchi.sh')
            file = open('../sim/modp.out.dpa', 'rb')
            data = np.fromfile(file, dtype='double').reshape(6, 8192)
            self.pschigraph.x, self.pschigraph.y = data[1, :], (
                data[0, :] - gam0) / sigg
...
#------------------------------------------------------------------------#
    ...
    def onAbout(self, event):
        ...
        aboutinfo.SetDescription(
            wordwrap(descriptionText, 500, wx.ClientDC(self)))
        aboutinfo.SetCopyright(u"(C) 2014-2015 Tong Zhang, SINAP, CAS")
        aboutinfo.SetWebSite(
            [u"http://210.72.8.51/FELwiki/index.php/DCLS", u"DCLS home page"])
...
#------------------------------------------------------------------------#
    ...
    def createPanel(self):
        ...
        sbox1 = wx.StaticBox(self.panel, id=wx.ID_ANY, label=u'Beam Matching')
        ### config staticbox
        sbox1.SetFont(sbfont)
        sbox1.SetForegroundColour(sbfontcolor)
        ### create the sizer
        ...
        layoutBitmap = wx.StaticBitmap(
            self.panel, wx.ID_ANY,
            wx.BitmapFromImage(
                funutils.rescaleImage(resutils.matchlayouticon.GetImage(),
                                      1.0)))
        sbsizer_layout.Add(layoutBitmap, proportion=1, flag=wx.ALIGN_CENTRE)
...
#------------------------------------------------------------------------#
    ...
    def createPanel(self):
        ...
        ## staticbox for detailed information of radiator line (layout)
        sbox1 = wx.StaticBox(self.panel, id=wx.ID_ANY, label=u'FEL Generation')
        ### config staticbox
        sbox1.SetFont(sbfont)
        sbox1.SetForegroundColour(sbfontcolor)
        ### create the sizer
        ...
        layoutBitmap = wx.StaticBitmap(
            self.panel, wx.ID_ANY,
            wx.BitmapFromImage(
                funutils.rescaleImage(resutils.radislayouticon.GetImage(),
                                      1.0)))
        sbsizer_layout.Add(layoutBitmap, proportion=1, flag=wx.ALIGN_CENTRE)
        ...
        hbox2.Add((5, -1))
        hbox2.Add(
            sbsizer_tapercontrol,
            proportion=1,
            flag=wx.EXPAND | wx.LEFT | wx.TOP | wx.BOTTOM,
            border=5)
        hbox2.Add(
            sbsizer_felgaincurve,
            proportion=1,
            flag=wx.EXPAND | wx.LEFT | wx.TOP | wx.BOTTOM,
            border=5)
        hbox2.Add(
            sbsizer_fieldpattern,
            proportion=1,
            flag=wx.EXPAND | wx.LEFT | wx.RIGHT | wx.TOP | wx.BOTTOM,
            border=5)
        hbox2.Add((5, -1))
...
#------------------------------------------------------------------------#
    ...
    def createPanel(self):
        ...
        sbox = wx.StaticBox(
            self.panel,
            id=wx.ID_ANY,
            label=u'(C) 2014-2015 Designed for DCLS',
            style=wx.ALIGN_LEFT)
        sbfont = wx.SystemSettings.GetFont(wx.SYS_SYSTEM_FONT)
        ...
        sboxparams = wx.StaticBox(
            self.panel,
            id=wx.ID_ANY,
            label=u'Parameters',
            style=wx.ALIGN_CENTER)
        sboximage = wx.StaticBox(
            self.panel,
            id=wx.ID_ANY,
            label=u'Laser Image',
            style=wx.ALIGN_CENTER)
        ...
        item11st = wx.StaticText(
            self.panel,
            label=u'Peak Power [W]' + ' ' + u'(\N{LATIN CAPITAL LETTER P})',
            style=wx.ALIGN_LEFT)
        item21st = wx.StaticText(
            self.panel,
            label=u'Pulse Width (FWHM) [fs]' + ' ' +
            u'(\N{GREEK SMALL LETTER TAU})',
            style=wx.ALIGN_LEFT)
        item31st = wx.StaticText(
            self.panel,
            label=u'Beam Waist Size [m]' + ' ' +
            u'(\N{GREEK SMALL LETTER OMEGA}\N{SUBSCRIPT ZERO})',
            style=wx.ALIGN_LEFT)
        #        item41st = wx.StaticText(self.panel, label = u'Rayleigh Range [m]'      + ' ' + u'(\N{LATIN CAPITAL LETTER Z}\N{LATIN SUBSCRIPT SMALL LETTER R})',
        #                style = wx.ALIGN_LEFT)
        item51st = wx.StaticText(
            self.panel,
            label=u'Phase [rad]' + ' ' + u'(\N{GREEK SMALL LETTER PHI})',
            style=wx.ALIGN_LEFT)
        item61st = wx.StaticText(
            self.panel,
            label=u'Wavelength [nm]' + ' ' + u'(\N{GREEK SMALL LETTER LAMDA})',
            style=wx.ALIGN_LEFT)

        self.item12tc = wx.TextCtrl(
            self.panel, id=ID_POWER, value='1.0e6', style=wx.TE_PROCESS_ENTER)
        self.item22tc = wx.TextCtrl(
            self.panel, id=ID_LENGTH, value='100', style=wx.TE_PROCESS_ENTER)
        self.item32tc = wx.TextCtrl(
            self.panel, id=ID_OMEGA, value='1.0e-3', style=wx.TE_PROCESS_ENTER)
        #        self.item42tc = wx.TextCtrl(self.panel, value = '1.0',    style = wx.TE_PROCESS_ENTER)
        self.item52tc = wx.TextCtrl(
            self.panel, id=ID_PHASE, value='0', style=wx.TE_PROCESS_ENTER)
        self.item62tc = wx.TextCtrl(
            self.panel, id=ID_WVLTH, value='300', style=wx.TE_PROCESS_ENTER)
        ...
        hboxbtn.Add(
            ...
            border=10)
        sbparams_sizer.Add(
            hboxbtn, flag=wx.ALIGN_CENTER | wx.LEFT | wx.RIGHT, border=10)

        #### laser image (transverse profile)
        vbox2 = wx.BoxSizer(orient=wx.VERTICAL)
        imgsrc_st = funutils.createStaticText(
            self.panel,
            label=u'Laser Source:',
            style=wx.ALIGN_LEFT,
            fontsize=10,
            fontweight=wx.FONTWEIGHT_NORMAL,
            fontcolor='BLACK')
        self.imgsrc_tc = wx.TextCtrl(
            self.panel, value='OPA:PROF:ARR', style=wx.TE_PROCESS_ENTER)
        self.lasergraph = pltutils.ImagePanel(
            self.panel, figsize=(4, 4), dpi=60, bgcolor=None)
        self.showbtn = wx.Button(self.panel, label='Single Shot')
        self.daqbtn = wx.Button(self.panel, label='DAQ START')
        imgcr_st = funutils.createStaticText(
            self.panel,
            label=u'Color Range:',
            style=wx.ALIGN_LEFT,
            fontsize=10,
        ...
        vbox2.Add(
            self.imgsrc_tc, flag=wx.TOP | wx.EXPAND | wx.ALIGN_LEFT, border=10)
        vbox2.Add(
            self.lasergraph, proportion=1, flag=wx.EXPAND | wx.ALL, border=10)
        ...
        hbox_fetchimg.Add(
            self.showbtn, proportion=0, flag=wx.RIGHT | wx.BOTTOM, border=10)
    ...
    def OnTextEnter(self, event):
        ...
        if self.imgpv.connected == False:
            dial = wx.MessageDialog(
                self,
                message=
                u"PV entered cannot be reached at this moment, please check the input name and try again later.",
    ...
    def OnSingleShot(self, event):
        ...
        try:
            ...
            dial = wx.MessageDialog(
                self,
                message=
                u"PV cannot be reached at this moment, please try again later.",
...
#------------------------------------------------------------------------#
    ...
    def createPanel(self):
        ...
        item11st = wx.StaticText(
            self.panel,
            label=u'Beam Energy [MeV]' + ' ' +
            u'(\N{LATIN CAPITAL LETTER E}\N{LATIN SMALL LETTER B})',
            style=wx.ALIGN_LEFT)
        item21st = wx.StaticText(
            self.panel,
            label=u'Energy Spread' + ' ' +
            u'(\N{GREEK SMALL LETTER SIGMA}\N{GREEK SMALL LETTER GAMMA})',
            style=wx.ALIGN_LEFT)
        item31st = wx.StaticText(
            self.panel,
            label=u'Beam Size [m]' + ' ' +
            u'(\N{GREEK SMALL LETTER SIGMA}\N{LATIN SUBSCRIPT SMALL LETTER R})',
            style=wx.ALIGN_LEFT)
        item51st = wx.StaticText(
            self.panel,
            label=u'Emittance [m]' + ' ' +
            u'(\N{GREEK SMALL LETTER EPSILON}\N{LATIN SMALL LETTER N})',
            style=wx.ALIGN_LEFT)
        item61st = wx.StaticText(
            self.panel,
            label=u'Peak Current [A]' + ' ' + u'(\N{LATIN CAPITAL LETTER I})',
            style=wx.ALIGN_LEFT)

        self.item12tc = wx.TextCtrl(
            self.panel, id=ID_ENERGY, value='840', style=wx.TE_PROCESS_ENTER)
        self.item22tc = wx.TextCtrl(
            self.panel,
            id=ID_ESPREAD,
            value='0.0001',
        ...
        self.item32tc = wx.TextCtrl(
            self.panel, id=ID_BSIZE, value='1.0e-4', style=wx.TE_PROCESS_ENTER)
        self.item52tc = wx.TextCtrl(
            self.panel, id=ID_EMITN, value='1.0e-6', style=wx.TE_PROCESS_ENTER)
        self.item62tc = wx.TextCtrl(
            self.panel, id=ID_CURPEAK, value='500', style=wx.TE_PROCESS_ENTER)
        ...
        imgsrc_st = funutils.createStaticText(
            self.panel,
            label=u'Profile Source:',
            style=wx.ALIGN_LEFT,
            fontsize=10,
            fontweight=wx.FONTWEIGHT_NORMAL,
            fontcolor='BLACK')
        self.imgsrc_tc = wx.TextCtrl(
            self.panel, value='DCLS:EBEAM:PROF:ARR', style=wx.TE_PROCESS_ENTER)
        self.ebeamgraph = pltutils.ImagePanel(
            self.panel, figsize=(4, 4), dpi=60, bgcolor=None)
        self.showbtn = wx.Button(self.panel, label='Single Shot')
        self.daqbtn = wx.Button(self.panel, label='DAQ START')
        imgcr_st = funutils.createStaticText(
        ...
        vbox2.Add(
            self.imgsrc_tc, flag=wx.TOP | wx.EXPAND | wx.ALIGN_LEFT, border=10)
        vbox2.Add(
            self.ebeamgraph, proportion=1, flag=wx.EXPAND | wx.ALL, border=10)
...
#------------------------------------------------------------------------#
    ...
    def createPanel(self):
        ...
        item11st = wx.StaticText(
            self.panel, label=u'Period Length [m]', style=wx.ALIGN_LEFT)
        item21st = wx.StaticText(
            self.panel, label=u'Period Number', style=wx.ALIGN_LEFT)
        item31st = wx.StaticText(
            self.panel, label=u'Undulator Gap [mm]', style=wx.ALIGN_LEFT)

        self.item12tc = wx.TextCtrl(
            self.panel, id=ID_MODXLAMD, value='50', style=wx.TE_PROCESS_ENTER)
        self.item22tc = wx.TextCtrl(
            self.panel, id=ID_MODNUM, value='10', style=wx.TE_PROCESS_ENTER)
        self.item32sc = wx.SpinCtrlDouble(
            self.panel,
            id=ID_MODGAP,
            value='',
...
#------------------------------------------------------------------------#
    ...
    def createPanel(self):
        ...
        item11st = wx.StaticText(
            self.panel, label=u'Dipole Length [m]', style=wx.ALIGN_LEFT)
        item21st = wx.StaticText(
            self.panel, label=u'Drift Length [m]', style=wx.ALIGN_LEFT)
        item31st = wx.StaticText(
            self.panel, label=u'Dipole Field [T]', style=wx.ALIGN_LEFT)
        item41st = wx.StaticText(
            self.panel, label=u'R56 [mm]', style=wx.ALIGN_LEFT)

        self.item12tc = wx.TextCtrl(
            self.panel, id=ID_IMAGL, value='0.150', style=wx.TE_PROCESS_ENTER)
        self.item22tc = wx.TextCtrl(
            self.panel, id=ID_IDRIL, value='0.285', style=wx.TE_PROCESS_ENTER)
        self.item32sc = wx.SpinCtrlDouble(
            self.panel,
            id=ID_IBFIELD,
            value='',
