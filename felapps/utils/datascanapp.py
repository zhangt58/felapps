...
class MainFrame(DataScanFrame):
    ...
    def save_mitemOnMenuSelection(self, event):
        ...
        if save_full_path is not None:
            scandata_ins = ScanDataFactor(
                self.scan_output_all, self.scaniternum_val, self.shotnum_val)
            ...
            rg.attrs[
                'timestamp'] = self.stop_timestamp  #time.strftime('%Y-%m-%d %H:%M:%S %Z', time.localtime())
            ...
            if data_fit is not None:
                rg.attrs['fit info'] = self.scanfig_panel.get_fit_model(
                ).fit_report()
                rg.attrs['fit data shape'] = data_fit.shape

            dset_list = [
                data_avgx, data_avgy, data_xerr, data_yerr, data_raw, data_fit
            ]
            dset_name = ['x_avg', 'y_avg', 'x_errb', 'y_errb', 'raw', 'fit']
            for (data, dsetname) in zip(dset_list, dset_name):
                if data is None: continue
                dset = f.create_dataset(
                    'data/' + dsetname, shape=data.shape, dtype=data.dtype)
                dset[...] = data
            f.close()

            dial = wx.MessageDialog(
                self,
                message="Saved Data into" + save_full_path + ".",
